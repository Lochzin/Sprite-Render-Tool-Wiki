...
- `Target Armature` (`target_armature`):
  - O objeto armadura cujas animações serão renderizadas.
- `is_previewing`:
  - Flag interno usado para saber se uma visualização de animação está sendo reproduzida atualmente.
- `custom_fps`:
...
O modo **Actions** usa uma lista customizada (`actions: CollectionProperty(type=ActionItem)`):
- Cada `ActionItem` tem:
...
- Auto-coletar NLA strips diretamente da armadura, ou
...
- Define `enabled` baseado em se a track e a strip estão mutadas.
...
- Muta todas as strips, depois desmuta exatamente a strip com o nome dado (e track opcional).
- Força uma atualização de objeto/view layer quando bem-sucedido.

Esta função é usada tanto para:
- Visualizações de teste (`test_nla_strip`), e
- Renderização real no modo NLA.

### Renderização no Modo NLA

Quando `Render All` executa com **NLA** selecionado:
...
- Antes de renderizar frames para uma animação dada:
  - O operador garante `animation_data.use_nla = True`.
  - Chama `activate_nla_strip` para desmutar apenas aquela strip.
- Então, como com Actions, itera sobre todos os frames e câmeras.
...
- **Stop Preview** (`sprite_render.stop_preview`):
  - Para a reprodução da timeline se estiver rodando.
...
- No modo **ACTIONS**, desabilite quaisquer actions que você não queira renderizar para reduzir o tempo total de renderização.
- No modo **NLA**, use a lista curada `nla_strips` quando quiser controle fino sobre quais strips são incluídas, independente de seu estado de mute no seu arquivo de trabalho.
