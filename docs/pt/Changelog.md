...
> **Nota:** Apenas a versão **0.2.1** está visível no snippet de código fornecido (`bl_info["version"] = (0, 2, 1)`). Versões mais antigas abaixo são exemplos; ajuste ou remova-as para corresponder ao seu histórico real.
...
- Primeira versão de tratamento por câmera (sem recursos completos de rotação de pivô).
...
**Mudanças:**
- Primeiro beta público do Sprite Render Tool.
- Renderização básica de sprites baseada em Actions.
- Painel UI inicial na sidebar da 3D Viewport.
