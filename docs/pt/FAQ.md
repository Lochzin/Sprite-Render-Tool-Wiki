...
- No 3D Viewport, use **Test Cameras** para ciclar pelas câmeras configuradas e confirmar qual está ativa.
- A câmera da cena está sendo definida corretamente pelo add-on (você deve ver a câmera mudar ao ciclar).

**P: A luz não rotaciona por câmera, ou parece errada. Como posso debugar isso?**  
**R:** Verifique o seguinte:
- Certifique-se de que um **Pivot Object** está definido na seção **Light Pivot**.
- Ative **Light Rotation Debug** na seção **Debug** para ver saída detalhada no console.
...
- No modo **NLA**:
  ...
  - Use **Preview NLA Strip** para ver se apenas aquela strip está ativa antes de renderizar.
...
- Estado `is_previewing`:
  ...
  - Use **Stop Preview** se a reprodução ficou travada ou se outra visualização está rodando.
...
**P: Mudei `Project Name` ou `Object Name` mas imagens antigas ainda estão na pasta anterior.**  
**R:** O add-on não move ou deleta renderizações antigas:
...
**R:** O add-on:
- Substitui caracteres que são inválidos em sistemas de arquivos comuns: `<>:"|?*\/` por `_`.
- Usa `os.path.basename` na saída do template para evitar caminhos aninhados em um único nome de arquivo.
Se você ver `_` extras, é provável que esteja sanitizando caracteres ou removendo barras acidentais no template.
...
- O **Render All** padrão do painel, que inicia o fluxo **async** baseado em timer, atualizando a UI entre frames e permitindo cancelamento.

**P: Como posso parar um lote longo de renderização?**  
...
Sempre leia a página `LICENSE.md` e **License.md** antes de redistribuir qualquer coisa.
...
- Licença Studio / Comercial: até 5 usuários por licença (extensível mediante solicitação).  
