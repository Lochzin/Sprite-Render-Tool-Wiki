...
Esta página cobre ferramentas de debug e padrões integrados no **Sprite Render Tool**.
...
Quando habilitado:
- O add-on imprime informações detalhadas no console do Blender sobre como a rotação do **objeto pivot** é aplicada:
  - Nome do objeto pivot.
  - Nome da câmera.
  - Valores de rotação solicitados.
  - Rotação original vs. nova rotação (em graus).
  - Qualquer erro que ocorra ao tentar aplicar a rotação.
...
Quando o debug de rotação de luz está ativo, para a primeira câmera na lista ordenada (ou a primeira câmera/frame no modo async) o add-on registra:
- Rotação Euler original do pivot (graus).
- A rotação desejada vinda de `cam_item.light_rotation`.
- Se a rotação XYZ completa está habilitada.
- A rotação final do pivot após aplicar a mudança.

No modo apenas Z:
- Ele registra Z original e novo Z em graus.

Se uma exceção ocorrer:
- Uma mensagem `DEBUG: Failed to rotate pivot 'Name': <error>` é reportada.

Use isso para:
- Confirmar que suas rotações de luz por câmera estão configuradas como esperado.
- Diagnosticar casos onde o pivot não está rotacionando ou está rotacionando no eixo/ordem errados.

---

## Progresso de Renderização e Cancelamento

Enquanto renderiza via `Render All` no modo async, o add-on expõe informações tipo debug na UI:
...
Se você precisar debugar lógica ao redor de:
...
- Se os arquivos estão aparecendo em pastas inesperadas:
  ...
  - Procure mensagens no console sobre falhas de criação de diretórios (se houver).
