...
- Nomes de pastas (indiretamente, via `$animation` e `$camera` quando opções de pasta estão habilitadas), e
...
- `$projectName`: tirado de `Project Name` na UI.
- `$objectName`: tirado de `Object Name` na UI.
- `$animation`: animação atual (nome da Action, nome da NLA strip, ou `"STATIC"` no modo Static).
- `$camera`: nome da câmera ou output name (veja docs de **Cameras**).
...
- Normaliza barras para o separador do OS.
- Divide em componentes de caminho.
- Remove o que parece um nome de arquivo ou "pasta falsa" no final, por exemplo:
...
- Se `settings` for fornecido, também remove pastas finais que correspondem a:
  - `project_name`
  - `object_name`
  - Qualquer nome de câmera ou output name potencial
    - Isso previne duplicar níveis `Project/Object/Camera` quando eles também são adicionados via opções de pasta.
...
O operador garante que diretórios existam em duas passadas:

1. **create_output_directories** (antes de renderizar):
   - Usa o modo de animação escolhido (Actions / NLA / Static) para coletar todos os nomes de animações.
...
- Constrói `final_dir` como descrito acima.
- Calcula `filename` de `output_template`:
...
- Isso previne que câmera A e câmera B produzam nomes de arquivos idênticos para os mesmos frames.

Se `$camera` **estiver** presente:
- O número do frame da timeline real é usado para `$frame`.
...
Isso pode ser útil se você está empacotando frames multi-câmera juntos em um pipeline customizado ou construindo sprite sheets programaticamente.
...
- Se você mudar opções de pasta no meio do projeto, esteja ciente de que novas renderizações podem ir para caminhos diferentes das mais antigas; mantenha os caminhos de importação do seu projeto de jogo sincronizados.
