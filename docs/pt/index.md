---
layout: default
title: "Sprite Render Tool — Wiki Oficial"
nav_order: 1
---

## Sprite Render Tool — Wiki Oficial

Add-on para **Blender** focado em gerar sprites 2D a partir de animações 3D, com **suporte a múltiplas câmeras**, **detecção automática de animações** (Actions e NLA), e **nomenclatura de arquivos altamente customizável**, ideal para jogos 2D e pipelines de sprites.
...
- **Suporta múltiplas câmeras**: você define uma lista de câmeras, ordem de renderização e nomes de saída customizados opcionais.
- **Controla iluminação via pivot**: usa um objeto pivot para rotacionar a "luz" em relação à câmera (ótimo para sprites isométricos/top-down).
...
- **Mostra progresso e permite cancelar**: barra de progresso integrada e botão de cancelar durante a renderização.
...
- **🎥 Câmeras**
  ...
  - `Custom Output Names`: usa nomes diferentes dos nomes dos objetos de câmera para a saída de arquivos.
  - `Enable Full Rotation (XYZ)`: controla se o pivot rotaciona em XYZ ou apenas ao redor de Z.
  - Para cada câmera (`Camera 1`, `Camera 2`, ...):
    ...
    - `Light Rotation`: rotação de luz/pivot (apenas Z ou XYZ).
  - Botão **Detect Cameras** (`sprite_render.autofill_light_rotation`):
    - Detecta câmeras visíveis na View Layer atual.
    - Preenche a lista com nomes, contagem e rotação Z baseada no objeto pivot.

- **💡 Light Pivot**
  - `Pivot Object`: objeto usado como pivot para rotação de luz.
...
- **💾 Output**
  ...
  - `Output Path` (das configurações de renderização da cena Blender): pasta base onde tudo será criado.

- **🐛 Debug**
  - `Light Rotation Debug`: imprime informações de debug sobre a rotação do pivot no console.
...
- **⚙️ Actions (Rodapé)**
  ...
  - `🎯 Test Cameras` (`sprite_render.test_cameras`):
    - Cicla pelas câmeras configuradas para visualização.
...
- **1. Prepare a cena**
  ...
  - Crie as câmeras que serão usadas para os sprites.
  - Opcionalmente crie um objeto `Pivot` para controlar a rotação de iluminação ao redor do personagem.
...
- **3. Escolha o modo de animação**
  - **ACTIONS**:
    ...
    - Ajuste `frame_start` e `frame_end` para cada action e habilite apenas as que você quer renderizar.
  - **NLA**:
...
- **4. Configure Output**
  - Ajuste `output_template` se quiser um padrão de nomenclatura de arquivos diferente.
...
- **Termos de distribuição comercial**:
  ...
  - Redistribuições de terceiros, mesmo que permitidas sob GPL, **não incluem suporte ou atualizações garantidas**.
  - Compartilhar o download comprado viola o acordo de compra.

Para detalhes completos, veja a página dedicada: **[Licença](License.md)**.

---

## Páginas da Wiki Sugeridas

Use esta página inicial como o índice principal. Páginas recomendadas:
...
- **Debug.md**: usando `Light Rotation Debug` e dicas comuns de debug.
